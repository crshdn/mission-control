...
export function TaskChatTab({ taskId }: TaskChatTabProps) {
  ...
  return (
    <div className="flex flex-col h-full min-h-[400px]">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 space-y-3">
        {notes.length === 0 && !waiting && (
          <div className="text-center py-12">
            <MessageSquare className="w-8 h-8 text-autensa-text-secondary mx-auto mb-3 opacity-50" />
            <p className="text-autensa-text-secondary text-sm">No messages yet</p>
            ...
            <div key={note.id} className={isAgent ? 'mr-8' : 'ml-8'}>
              <div className={`border rounded-lg px-3 py-2 ${
                ...
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs font-medium text-autensa-text-secondary">
                    {isAgent ? 'Agent' : 'You'}
                  </span>
                  ...
                  <span className="ml-auto text-xs text-autensa-text-secondary/50">
                    {new Date(note.created_at.endsWith('Z') ? note.created_at : note.created_at + 'Z').toLocaleTimeString()}
                  </span>
                </div>
                <div className="text-sm text-autensa-text whitespace-pre-wrap">{note.content}</div>
              </div>
            </div>
          );
        })}
        ...
        {waiting && (
          <div className="mr-8">
            <div className="bg-green-500/10 border border-green-500/20 rounded-lg px-3 py-2 inline-flex items-center gap-2">
              <span className="text-xs font-medium text-autensa-text-secondary">Agent</span>
      ...
      <div className="border-t border-autensa-border p-3 space-y-2">
        {error && (
