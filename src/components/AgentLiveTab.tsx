...
export function AgentLiveTab({ taskId }: AgentLiveTabProps) {
  ...
  const roleStyles: Record<string, { bg: string; label: string; align: string }> = {
    ...
    system: { bg: 'bg-autensa-bg-tertiary border-autensa-border', label: 'System', align: 'mx-8' },
  };
  ...
  return (
    <div className="flex flex-col h-full min-h-[400px]">
      {/* Status bar */}
      <div className="flex items-center gap-2 px-3 py-2 border-b border-autensa-border text-xs">
        {status === 'streaming' && (
          <>
        ...
        {status === 'connecting' && (
          <>
            <Wifi className="w-3.5 h-3.5 text-autensa-text-secondary animate-pulse" />
            <span className="text-autensa-text-secondary">Connecting...</span>
          </>
        )}
        {status === 'no_session' && (
          <>
            <WifiOff className="w-3.5 h-3.5 text-autensa-text-secondary" />
            <span className="text-autensa-text-secondary">No active session — dispatch the task to see live activity</span>
          </>
        )}
        ...
        {(messages.length > 0 || activeStreams.length > 0) && (
          <span className="ml-auto text-autensa-text-secondary">{messages.length} messages</span>
        )}
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 space-y-3">
        {messages.length === 0 && activeStreams.length === 0 && status !== 'no_session' && (
          <div className="text-center py-8 text-autensa-text-secondary text-sm">
        ...
        {status === 'no_session' && messages.length === 0 && (
          <div className="text-center py-12">
            <Radio className="w-8 h-8 text-autensa-text-secondary mx-auto mb-3 opacity-50" />
            <p className="text-autensa-text-secondary text-sm">No active agent session</p>
            <p className="text-autensa-text-secondary/60 text-xs mt-1">Dispatch the task to start streaming agent activity</p>
          </div>
        )}
            ...
            <div key={msg.index} className={`${style.align}`}>
              <div className={`${style.bg} border rounded-lg px-3 py-2`}>
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs font-medium text-autensa-text-secondary">{style.label}</span>
                  {msg.timestamp && (
                    <span className="text-xs text-autensa-text-secondary/50">
                      {new Date(msg.timestamp).toLocaleTimeString()}
                    </span>
                  )}
                </div>
                <div className="text-sm text-autensa-text whitespace-pre-wrap break-words">
                  {msg.content}
                </div>
              </div>
            </div>
          ...
          return (
            <div key={`stream-${streamType}`} className="mr-8">
              <div className={`${style.bg} border rounded-lg px-3 py-2`}>
                ...
                <div className="text-sm text-autensa-text whitespace-pre-wrap break-words font-mono">
                  {content}
                </div>
              </div>
            </div>
