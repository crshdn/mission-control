...
export function SessionsList({ taskId }: SessionsListProps) {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'completed':
        return <CheckCircle className="w-4 h-4 text-autensa-accent" />;
      case 'failed':
        return <XCircle className="w-4 h-4 text-red-500" />;
      default:
        return <Circle className="w-4 h-4 text-autensa-text-secondary" />;
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-8">
        <div className="text-autensa-text-secondary">Loading sessions...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      {sessions.map((session) => (
        <div
          key={session.id}
          className="flex gap-3 p-3 bg-autensa-bg rounded-lg border border-autensa-border"
        >
          {/* Agent Avatar */}
          <div className="flex-shrink-0">
            {session.agent_avatar_emoji ? (
              ...
              <Bot className="w-8 h-8 text-autensa-accent" />
            )}
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="font-medium text-autensa-text">
                {session.agent_name || 'Sub-Agent'}
              </span>
              <span className="text-xs text-autensa-text-secondary capitalize">
                {session.status}
              </span>
            </div>

            {/* Session ID */}
            <div className="text-xs text-autensa-text-secondary font-mono mb-2 truncate">
            ...
            {/* Channel */}
            {session.channel && (
              <div className="mt-2 text-xs text-autensa-text-secondary">
          ...
          <div className="flex flex-col gap-1">
            {session.status === 'active' && (
              <button
                ...
                className="p-1.5 hover:bg-autensa-bg-tertiary rounded text-green-500"
            ...
            <button
              ...
              className="p-1.5 hover:bg-autensa-bg-tertiary rounded text-red-500"
