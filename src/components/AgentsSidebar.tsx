...
export function AgentsSidebar({ workspaceId, mobileMode = false, isPortrait = true }: AgentsSidebarProps) {
  ...
  return (
    <aside
      className={`bg-autensa-bg-secondary ${mobileMode ? 'border border-autensa-border rounded-lg h-full' : 'border-r border-autensa-border'} flex flex-col transition-all duration-300 ease-in-out ${
    ...
    >
        ...
        <div className="flex items-center">
          ...
          {!effectiveMinimized && (
            <>
              ...
              <span className="bg-autensa-bg-tertiary text-autensa-text-secondary text-xs px-2 py-0.5 rounded ml-2">{agents.length}</span>
            </>
          )}
        </div>
            ...
            {activeSubAgents > 0 && (
              <div className="mb-3 mt-3 px-3 py-2 bg-green-500/10 border border-green-500/20 rounded-lg">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-autensa-text">Active Sub-Agents:</span>
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1">
        {filteredAgents.map((agent) => {
            ...
            return (
              <div key={agent.id} className="flex justify-center py-3">
                ...
                >
                  ...
                  {openclawSession && <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-green-500 rounded-full border-2 border-autensa-bg-secondary" />}
                  {!!agent.is_master && <span className="absolute -top-1 -right-1 text-xs text-autensa-accent-yellow">★</span>}
                  <span
                    className={`absolute -bottom-1 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full ${
                      agent.status === 'working' ? 'bg-autensa-accent-green' : agent.status === 'standby' ? 'bg-autensa-text-secondary' : 'bg-gray-500'
                    }`}
                  />
                  <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 px-2 py-1 bg-autensa-bg text-autensa-text text-xs rounded opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-50 border border-autensa-border">
                    {agent.name}
                  </div>
                </button>
              </div>
          ...
          return (
            <div key={agent.id} className={`w-full rounded hover:bg-autensa-bg-tertiary transition-colors ${selectedAgent?.id === agent.id ? 'bg-autensa-bg-tertiary' : ''}`}>
              <button
                onClick={() => {
                  setSelectedAgent(agent);
                  setEditingAgent(agent);
              ...
              >
                <div className="text-2xl relative">
                  {agent.avatar_emoji}
                  {openclawSession && <span className="absolute -bottom-1 -right-1 w-3 h-3 bg-green-500 rounded-full border-2 border-autensa-bg-secondary" />}
                </div>

                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-sm truncate">{agent.name}</span>
                    {!!agent.is_master && <span className="text-xs text-autensa-accent-yellow">★</span>}
                  </div>
                  <div className="text-xs text-autensa-text-secondary truncate flex items-center gap-1">
                    {agent.role}
                  ...
                  <button
                    ...
                    className={`w-full min-h-11 flex items-center justify-center gap-2 px-2 rounded text-xs transition-colors ${
                      openclawSession
                        ...
                        : 'bg-autensa-bg text-autensa-text-secondary hover:bg-autensa-bg-tertiary hover:text-autensa-text'
                    }`}
                  >
                    {isConnecting ? (
                      <>
      ...
      {!effectiveMinimized && (
        <div className="p-3 border-t border-autensa-border space-y-2">
          <button
            onClick={() => setShowCreateModal(true)}
            className="w-full min-h-11 flex items-center justify-center gap-2 px-3 bg-autensa-bg-tertiary hover:bg-autensa-border rounded text-sm text-autensa-text-secondary hover:text-autensa-text transition-colors"
          >
