  ...
  return (
    <div className="bg-autensa-bg-secondary border border-autensa-border rounded-lg p-5 space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-autensa-text text-sm">Cost Caps</h3>
        <button
          onClick={() => setShowCreate(!showCreate)}
          className="text-xs px-3 py-1.5 rounded bg-autensa-accent/20 text-autensa-accent hover:bg-autensa-accent/30 flex items-center gap-1"
        >
      ...
      {showCreate && (
        <div className="flex items-end gap-3 p-3 bg-autensa-bg rounded-lg">
          <div className="flex-1">
            <label className="block text-xs text-autensa-text-secondary mb-1">Type</label>
            <select
              value={newCap.cap_type}
              onChange={e => setNewCap(n => ({ ...n, cap_type: e.target.value }))}
              className="w-full bg-autensa-bg-tertiary border border-autensa-border rounded px-3 py-2 text-sm text-autensa-text"
            >
          ...
          <div className="flex-1">
            <label className="block text-xs text-autensa-text-secondary mb-1">Limit (USD)</label>
            <input
              type="number"
          ...
          <button
            ...
            className="min-h-[38px] px-4 rounded bg-autensa-accent text-white text-sm hover:bg-autensa-accent/90"
          >
            Create
          </button>
        </div>
      ...
      {loading ? (
        <div className="text-sm text-autensa-text-secondary animate-pulse">Loading...</div>
      ) : caps.length === 0 ? (
        <div className="text-sm text-autensa-text-secondary">No cost caps configured</div>
      ) : (
        <div className="space-y-2">
          {caps.map(cap => {
            ...
            return (
              <div key={cap.id} className="flex items-center gap-3 p-3 bg-autensa-bg rounded-lg">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-sm font-medium text-autensa-text">{capTypeLabels[cap.cap_type] || cap.cap_type}</span>
                ...
                <button
                  ...
                  className="p-1.5 text-autensa-text-secondary hover:text-red-400"
                >
