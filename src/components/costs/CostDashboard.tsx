...
export function CostDashboard({ productId, workspaceId = 'default' }: CostDashboardProps) {
  ...
  if (loading) {
    return <div className="text-autensa-text-secondary animate-pulse">Loading costs...</div>;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {overview && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            <div key={item.label} className="bg-autensa-bg-secondary border border-autensa-border rounded-lg p-4">
              <div className="text-xs text-autensa-text-secondary uppercase mb-1">{item.label}</div>
              <div className="text-xl font-bold text-autensa-text">{formatUsd(item.value)}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {breakdown && breakdown.by_event_type.length > 0 && (
        <div className="bg-autensa-bg-secondary border border-autensa-border rounded-lg p-5">
          <h3 className="font-semibold text-autensa-text mb-4">Cost by Category</h3>
            ...
            {breakdown.by_event_type.map(item => {
              ...
              return (
                <div key={item.event_type} className="flex items-center gap-3">
                  <span className="text-sm text-autensa-text-secondary w-32 capitalize truncate">{label}</span>
                  <div className="flex-1 h-5 bg-autensa-bg-tertiary rounded overflow-hidden">
                    <div
                      className="h-full bg-autensa-accent-cyan/60 rounded"
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                  <span className="text-sm font-medium text-autensa-text w-20 text-right">{formatUsd(item.total)}</span>
                  <span className="text-xs text-autensa-text-secondary w-12 text-right">({item.count})</span>
                </div>
              );
            })}
          </div>
      ...
      {breakdown?.per_feature && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-autensa-bg-secondary border border-autensa-border rounded-lg p-4">
            <div className="text-xs text-autensa-text-secondary mb-1">Avg Cost / Idea</div>
            <div className="text-lg font-bold text-autensa-text">{formatUsd(breakdown.per_feature.avg_cost_per_idea)}</div>
          </div>
          <div className="bg-autensa-bg-secondary border border-autensa-border rounded-lg p-4">
            <div className="text-xs text-autensa-text-secondary mb-1">Avg Cost / Feature</div>
            <div className="text-lg font-bold text-autensa-text">{formatUsd(breakdown.per_feature.avg_cost_per_shipped_feature)}</div>
          </div>
          <div className="bg-autensa-bg-secondary border border-autensa-border rounded-lg p-4">
            <div className="text-xs text-autensa-text-secondary mb-1">Total Ideas Cost</div>
            <div className="text-lg font-bold text-autensa-text">{formatUsd(breakdown.per_feature.total_ideas_cost)}</div>
          </div>
          <div className="bg-autensa-bg-secondary border border-autensa-border rounded-lg p-4">
            <div className="text-xs text-autensa-text-secondary mb-1">Total Build Cost</div>
            <div className="text-lg font-bold text-autensa-text">{formatUsd(breakdown.per_feature.total_build_cost)}</div>
          </div>
        </div>
      )}
      ...
      {productId && (
        <div className="bg-autensa-bg-secondary border border-autensa-border rounded-lg p-5 space-y-4">
          <h3 className="font-semibold text-autensa-text">Product Cost Caps</h3>
          ...
          {productCaps.cost_cap_monthly && overview && (
            <div>
              <div className="flex justify-between text-xs text-autensa-text-secondary mb-1">
                <span>Monthly spend</span>
                <span>{formatUsd(overview.this_month)} / {formatUsd(productCaps.cost_cap_monthly)}</span>
              </div>
              <div className="h-2 bg-autensa-bg-tertiary rounded overflow-hidden">
                <div
                  className={`h-full rounded ${capExceeded ? 'bg-amber-500' : 'bg-autensa-accent-cyan/60'}`}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-autensa-text-secondary mb-1">Per-task cap ($)</label>
              <input
                type="number"
                min="0"
                step="0.01"
                ...
                className="w-full bg-autensa-bg-tertiary border border-autensa-border rounded-lg px-3 py-2 text-autensa-text text-sm focus:outline-none focus:border-autensa-accent"
                placeholder="No limit"
              />
            </div>
            <div>
              <label className="block text-xs text-autensa-text-secondary mb-1">Monthly cap ($)</label>
              <input
                type="number"
                min="0"
                step="0.01"
          ...
          <button
            ...
            disabled={savingCaps}
            className="text-sm px-4 py-2 bg-autensa-accent text-white rounded-lg hover:bg-autensa-accent/90 disabled:opacity-50"
          >
      ...
      <div>
        <button
          ...
          className="text-sm text-autensa-accent hover:text-autensa-accent/80 mb-3"
        >
